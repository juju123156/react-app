...
//            }
	...
	searchL2Component: function() {
		...
		$('#jqGridL2').jqGrid({
			...
			,colNames:['본부','팀', 'ONS팀','전송로유형','망사업자','IP 지역','IP','MAC','시리얼넘버','장비명','연결AP개수','연동상태','Trap 수신 시간','제조사','모델명','S/W 버전','H/W 버전', 'L2 상세위치','특이사항','우편번호','도로명주소','상세주소','설치업체']
			,colModel: [
				{ name: 'GROUP_1_NAME', index: 'GROUP_1_NAME', width: 75, align: 'center' },
				{ name: 'GROUP_2_NAME', index: 'GROUP_2_NAME', width: 150 },
				{ name: 'GROUP_3_NAME', index: 'GROUP_3_NAME', width: 150 },
				{ name: 'TRANS_TYPE_STR', index: 'TRANS_TYPE_STR', width: 80, align: 'center' },
				{ name: 'TLINE_BIZ_NM_STR', index: 'TLINE_BIZ_NM_STR', width: 80, align: 'center' },
				...
				{ name: 'AP_CNT', index: 'AP_CNT', width: 80 , align: 'center'},
				{ name: 'ALARMSTATE', index: 'ALARMSTATE', width: 80, align: 'center' },
				...
				{ name: 'L2_MANAGER_COMPANY', index: 'L2_MANAGER_COMPANY', width: 150, hidden :true }
			]
			,pager : false
			//,loadonce: true
		...
		resizeJqGridWidth('jqGridL2', 'mu-grid-body', $('.mu-grid-body').width(), true); //사이즈 조정 함수 . 아래 첨부
		/*
...
var popup = {	
		registerL2Componet:function(){
			...
			if(rowid == null) {
				...
				var l2_manager_company = $("#jqGridL2").jqGrid("getRowData", rowid).L2_MANAGER_COMPANY;
				...
				$('<div id="l2componentpopup">').dialog({
		            ...
		            open: function (e)
		            {
		            	...
		            	$(this).load('/views/l2/popup/l2componentpopup.jsp', function(){ 
		            		...
		            		$("#managercomp").val(l2_manager_company);
...
function setComboState ( div )
{
	...
	div.append('<li value="0">수신</li>');
	div.append('<li value="4">미수신</li>');
