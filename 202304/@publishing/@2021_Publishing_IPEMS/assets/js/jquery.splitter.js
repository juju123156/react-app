        ...
        if (splitters.filter(Boolean).length === 0) {
            ...
            $(document.documentElement).on('mousedown.splitter touchstart.splitter', function(e) {
                ...
                if (current_splitter) {
					...
					// rnms
					if ($('.splitter-top .top-box').height() <= 50) {
						$('.splitter-top .top-box .btn-click').removeClass('on');
					} else if ($('.splitter-top .top-box').height() >= 270) {
						$('.splitter-top .top-box .btn-click').addClass('on');
					}
                }
