...
var Path = /** @class */ (function () {
     ...
     */
    Path.prototype.arcTo = function (rx, ry, rotation, fA, fS, x2, y2) {
    ...
    Path.prototype.arcToAlt = function (rx, ry, rotation, fA, fS, x2, y2) {
     ...
     */
    Path.cubicArc = function (commands, params, cx, cy, rx, ry, phi, theta1, theta2, anticlockwise) {
        if (anticlockwise) {
            var temp = theta1;
            theta1 = theta2;
            theta2 = temp;
    ...
    Path.prototype.cubicArc = function (cx, cy, rx, ry, phi, theta1, theta2, anticlockwise) {
     ...
     * @param t
     */
    Path.prototype.deCasteljau = function (points, t) {
        var n = points.length;
        if (n < 2 || n % 2 === 1) {
     ...
     * @param n
     */
    Path.prototype.approximateCurve = function (points, n) {
        this.moveTo.apply(this, this.deCasteljau(points, 0));
        var step = 1 / n;
        for (var t = step; t <= 1; t += step) {
            this.lineTo.apply(this, this.deCasteljau(points, t));
        }
    };
    /**
     ...
     * @param value
     */
    Path.parseSvgPath = function (value) {
        return value.trim().split(Path.splitCommandsRe).map(function (part) {
            var strParams = part.match(Path.matchParamsRe);
            return {
    ...
    Path.prettifySvgPath = function (value) {
        return Path.parseSvgPath(value).map(function (d) { return d.command + d.params.join(','); }).join('\n');
    };
    /**
     * See https://www.w3.org/TR/SVG11/paths.html
     * @param value
     */
    Path.prototype.setFromString = function (value) {
        var _this = this;
        this.clear();
        var parts = Path.parseSvgPath(value);
        // Current point.
        var x;
        var y;
    ...
    Path.prototype.toPrettyString = function () {
    ...
    Path.prototype.toDebugSvg = function () {
        var d = Path.prettifySvgPath(this.toString());
     ...
     */
    Path.prototype.toCubicPaths = function () {
    ...
    Path.cubicPathToString = function (path) {
        var n = path.length;
        if (!(n % 2 === 0 && (n / 2 - 1) / 2 >= 1)) {
    ...
    Path.splitCommandsRe = /(?=[AaCcHhLlMmQqSsTtVvZz])/g;
    Path.matchParamsRe = /-?[0-9]*\.?\d+/g;
    Path.quadraticCommandRe = /[QqTt]/;
    Path.cubicCommandRe = /[CcSs]/;
    Path.xmlDeclaration = '<?xml version="1.0" encoding="UTF-8"?>';
